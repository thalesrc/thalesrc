# Messenger Test Commands - Quick Reference

## Most Common Commands

```bash
# Run all tests (all submodules in parallel) - Use in CI
pnpm nx run Messenger:test

# Run specific submodule tests
pnpm nx run Messenger:test/core        # Core functionality (Node.js)
pnpm nx run Messenger:test/worker      # Web Worker tests (Browser)

# Run tests with coverage and merge reports
pnpm nx run Messenger:test/coverage
```

## All Available Commands

| Command | Description | Environment | Speed |
|---------|-------------|-------------|-------|
| `pnpm nx run Messenger:test` | All submodules (parallel) | Mixed | Medium |
| `pnpm nx run Messenger:test/core` | Core functionality | Node | Fast |
| `pnpm nx run Messenger:test/worker` | Web Worker APIs | Browser (Chromium) | Medium |
| `pnpm nx run Messenger:test/chrome` | Chrome extensions | Browser + Chrome APIs | Medium |
| `pnpm nx run Messenger:test/broadcast` | Broadcast Channel | Browser (Chromium) | Medium |
| `pnpm nx run Messenger:test/iframe` | iframe communication | Browser (Chromium) | Medium |
| `pnpm nx run Messenger:test/coverage` | All tests + merge coverage | Mixed | Slowest |
| `pnpm nx run Messenger:test/prepare/browser` | Install Playwright browsers | - | Slow |

## Watch Mode

```bash
# Core tests in watch mode
pnpm nx run Messenger:test/core -- --watch

# Worker tests in watch mode with UI
pnpm nx run Messenger:test/worker -- --watch --ui

# All tests in watch mode
pnpm nx run Messenger:test -- --watch
```

## Coverage

```bash
# Run single submodule with coverage
pnpm nx run Messenger:test/core:with-coverage
pnpm nx run Messenger:test/worker:with-coverage

# Run all tests with coverage and merge
pnpm nx run Messenger:test/coverage

# View coverage reports
open coverage/libs/messenger/core/index.html
open coverage/libs/messenger/worker/index.html
open coverage/libs/messenger/merged/index.html  # Merged report
```

## CI/CD Examples

```bash
# GitHub Actions - Install Playwright first
pnpm nx run Messenger:test/prepare/browser

# GitHub Actions - Run all tests with coverage
pnpm nx run Messenger:test/coverage

# Pre-commit hook - Only affected
pnpm nx run Messenger:test

# Manual verification before PR
pnpm nx run Messenger:test/coverage
```

## Test File Patterns

Tests are organized by submodule location:

- `src/**/*.spec.ts` (excluding submodules) → `test/core`
- `src/worker/**/*.spec.ts` → `test/worker`
- `src/chrome/**/*.spec.ts` → `test/chrome`
- `src/broadcast/**/*.spec.ts` → `test/broadcast`
- `src/iframe/**/*.spec.ts` → `test/iframe`

Both `*.spec.ts` and `*.test.ts` suffixes are supported.

## Common Issues

**Browser tests timeout**: Increase timeout in the submodule's vitest.config.ts
```typescript
test: {
  testTimeout: 10000, // 10 seconds
}
```

**Playwright not installed**: Run installer
```bash
pnpm nx run Messenger:test/prepare/browser
# or
pnpm exec playwright install chromium --with-deps
```

**Tests not found**: 
- Check file is in the correct submodule directory
- Verify file has `.spec.ts` or `.test.ts` suffix
- Check the submodule's vitest.config.ts include pattern

**Coverage not generated**:
- Use `test/coverage` target for merged coverage
- Or use `test/{submodule}:with-coverage` for individual submodule
- Check that `@vitest/coverage-istanbul` and `@vitest/coverage-v8` are installed

**Wrong coverage provider error**:
- Browser tests must use `istanbul` provider
- Node.js tests must use `v8` provider
- This is already configured correctly in each submodule's config

## Environment Variables

```bash
# Run browser tests in visible mode (for debugging)
HEADLESS=false pnpm nx run Messenger:test/worker

# Or use the dedicated headed target
pnpm nx run Messenger:test/headed
```

## Passing Additional Vitest Options

```bash
# Pass options after --
pnpm nx run Messenger:test/worker -- --reporter=verbose

# Run specific test file
pnpm nx run Messenger:test/worker -- message-client.spec.ts

# Run tests matching a pattern
pnpm nx run Messenger:test/worker -- -t "should send message"

# Run with browser headless override
pnpm nx run Messenger:test/worker -- --browser.headless=false
```

---

For detailed information, see [TESTING-GUIDE.md](./TESTING-GUIDE.md)
