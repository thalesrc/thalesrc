# Messenger Testing Guide

This document explains the testing structure for the Messenger library, which is organized by submodules for modular testing and coverage tracking.

## Overview

The Messenger library uses a modular testing setup where each submodule has its own testing configuration, but all tests can be run from a single unified task.
...
pnpm nx run Messenger:test/core

# Worker tests (Browser)
pnpm nx run Messenger:test/worker

# Chrome extension tests
pnpm nx run Messenger:test/chrome

# Broadcast tests
pnpm nx run Messenger:test/broadcast

# Iframe tests
pnpm nx run Messenger:test/iframe
```
...
- Core: `coverage/libs/messenger/core/`
- Worker: `coverage/libs/messenger/worker/`
- Chrome: `coverage/libs/messenger/chrome/`
- Broadcast: `coverage/libs/messenger/broadcast/`
- Iframe: `coverage/libs/messenger/iframe/`
...
3. Merge all reports into `coverage/libs/messenger/merged/`
...
export default defineConfig(() => ({
  ...
  cacheDir: '../../../node_modules/.vite/libs/messenger-yourmodule',
  ...
  test: {
    ...
    coverage: {
      clean: true,
      reportsDirectory: path.join(__dirname, '../../../coverage/libs/messenger/yourmodule'),
      provider: 'v8',
...
- name: Upload coverage to Codecov
  ...
  with:
    directory: ./coverage/libs/messenger/
    flags: Messenger
...
```bash
pnpm nx run Messenger:test/coverage  # Coverage is enabled automatically
```
